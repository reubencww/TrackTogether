...
import com.example.tracktogether.face.model.MaskDetectionModel
...
class FrameAnalyzer(
    ...
    // <-------------------------------------------------------->
    private val TAG = FrameAnalyzer::class.java.simpleName
    ...
    private suspend fun runModel(faces: List<Face>, cameraFrameBitmap: Bitmap) {
        withContext(Dispatchers.Default) {
                ...
                for (face in faces) {
                    try {
                        ...
                        if (maskLabel == maskDetectionModel.noMask) {
                            ...
                            Log.v(TAG, "Average score for each user : $nameScoreHashmap")
                            ...
                            Log.v(TAG, "Person identified as $bestScoreUserName")
            ...
            withContext(Dispatchers.IO) {
                ...
                if (validCount > countLimit) {
                    for (prediction in predictions) {
                        if (!photoTaken) {
                            photoTaken = true
                            RecognitionActivity.getCameraInstanceActivity()?.takePhoto()
                        }
                    }
                }
